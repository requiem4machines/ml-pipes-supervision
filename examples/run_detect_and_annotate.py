"""
RF-DETR detection annotated through Roboflow Inference and Supervision.

Run from the repo root:
    python examples/run_detect_and_annotate.py
    python examples/run_detect_and_annotate.py --input path/to/photo.jpg
    python examples/run_detect_and_annotate.py --model-id rfdetr-medium
"""
...
def build_pipeline(
    model_id: str,
    api_key: str | None,
) -> Pipeline[str | Path, tuple[npt.NDArray[np.uint8], supervision.Detections]]:
    return Pipeline(
        [
            LoadFile(),
            Decode(),
